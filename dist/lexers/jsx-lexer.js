  ...
  _createClass(JsxLexer, [
    ...
    },
    {
      ...
      value: function jsxExtractor(node, sourceText) {
        ...
        var getPropValue = function getPropValue(node, attributeName) {
          var _attribute$initialize
          var attribute = node.attributes.properties.find(function (attr) {
            return attr.name !== undefined && attr.name.text === attributeName
          })
        ...
        if (tagNode.tagName.text === 'Trans') {
          ...
          tagNode.attributes.properties.forEach(function (property) {
            if (property.kind === ts.SyntaxKind.JsxSpreadAttribute) {
              _this3.emit(
                'warning',
                'Component attribute is a JSX spread attribute : '.concat(
